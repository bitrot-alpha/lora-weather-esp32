...
void api_test(void);
bool load_cert_file(const char * path, String & content);
...
void create_main_gui(void)
{
    ...
    lv_obj_align_to(text_label_temperature, thermometer_img, LV_ALIGN_OUT_RIGHT_MID, 12,0);
    ...
    lv_obj_align_to(text_label_humidity, droplet_img, LV_ALIGN_OUT_RIGHT_MID, 12, 0);

    lv_obj_set_style_text_font(text_label_time, &clock_132, 0);
    lv_obj_set_style_bg_color(text_label_time, lv_color_black(), 0);
    lv_obj_set_style_bg_opa(text_label_time, LV_OPA_COVER, 0);
    lv_obj_set_style_pad_all(text_label_time, 12, 0);
    lv_obj_set_style_text_color(text_label_time, lv_palette_main(LV_PALETTE_LIGHT_GREEN),0);
    lv_label_set_text(text_label_time, "XX:XX");
    lv_obj_align(text_label_time, LV_ALIGN_TOP_MID, -78, 0);
...
void api_test(void)
{
  //format of api_key.h
  /*
  const char apikey[] = "YOUR_PIRATEWEATHER_KEY";
  //lat and long are only accurate to 3 decimal places
  //api can accept more precision, but it's pointless
  const char latitude[] = "37.422";
  const char longitude[] = "-122.084";
  */
  char url[160] = "https://api.pirateweather.net/forecast/";
  strcat(url, apikey);
  strcat(url, "/");
  strcat(url, latitude);
  strcat(url, ",");
  strcat(url, longitude);
  strcat(url, "?exclude=minutely");
  strcat(url, "&units=us");

  Serial.printf("Getting from: %s\r\n", url);
  Serial.println("If WifiClientSecure complains, Root CA cert is wrong or missing.");

  WiFiClientSecure wifi;
  String cert;

  if (load_cert_file("/pirate_weather_root_cert.pem", cert) )
  {
  ...
  {  
    // Step 1: Create a filter to specify which fields to keep  
    JsonDocument filter;  
    ...
    filter["hourly"]["data"][0]["summary"] = true;  
      
    // Step 2: Deserialize with the filter  
    // Size estimate: you only need ~1KB since you're filtering most data  
    JsonDocument doc;  
    ...
    // Current conditions  
    long currentTime = doc["currently"]["time"];  
    const char* currentIcon = doc["currently"]["icon"];  
    const char* currentSummary = doc["currently"]["summary"];

    //convert epoch to human time
    struct tm* tm_info = gmtime(&currentTime);
    char conv_time[32] = "";
    strftime(conv_time, sizeof(conv_time), "%Y-%m-%d %H:%M:%S", tm_info);
      
    Serial.println("=== Current Conditions ===");  
    Serial.printf("Time: %ld %s UTC\n", currentTime, conv_time);  
    Serial.printf("Icon: %s\n", currentIcon);  
    Serial.printf("Summary: %s\n", currentSummary);  
      
    // Hourly forecast (first 6 hours)  
    Serial.println("\n=== Hourly Forecast ===");  
    JsonArray hourlyData = doc["hourly"]["data"];  
    int count = 0;  
    for (JsonObject hour : hourlyData) {  
      if (count >= 6) break;  // Only process first 6 hours  
        
      long time = hour["time"];
      struct tm * tm_info = gmtime(&time);
      const char* icon = hour["icon"];  
      const char* summary = hour["summary"];  
        
      Serial.printf("Hour %d - Time: %ld %02d:%02d, Icon: %s, Summary: %s\n",   
                    count + 1, time, tm_info->tm_hour, tm_info->tm_min, icon, summary);  
      count++;  
    }  
  } 
  else
...
bool load_cert_file(const char * path, String & content)
{
  ...
  content = f.readString();  // read whole file (!)
  f.close();

  Serial.printf("Loaded cert %s (%d bytes)\n", path, content.length());
  return true;
}
